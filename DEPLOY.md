...
| Carpeta del proyecto | Fuera de `public_html`, en el home del usuario — confirma el nombre exacto en tu Administrador de Archivos (a lo largo de este proyecto nos referimos a ella como `vitalclean-app`, ajusta si la llamaste distinto) |
| Acceso SSH/Composer | No disponible — todo se sube manualmente |
...
carpeta (`.../vitalclean-app/public`), o bien el contenido de `public/` se
...
2. Cambia el Document Root a:
   ```
   /home/wojttyoo/vitalclean-app/public
   ```

Si tu plan **no** permite cambiar el Document Root, la alternativa es
copiar el *contenido* de `vitalclean-app/public/` (no la carpeta, su
...
`__DIR__.'/../vitalclean-app/vendor/autoload.php'` y
`__DIR__.'/../vitalclean-app/bootstrap/app.php'`.

### 1.3 Configurar `.env`

Dentro de `vitalclean-app/`, copia `.env.example` a `.env` y edítalo:
...
- `vitalclean-app/storage/`
- `vitalclean-app/bootstrap/cache/`
...
2. Sube ese zip a la **raíz de tu carpeta del proyecto**
   (`vitalclean-app/`), no dentro de `public_html`.
...
En el Administrador de Archivos, entra a `vitalclean-app/bootstrap/cache/`
